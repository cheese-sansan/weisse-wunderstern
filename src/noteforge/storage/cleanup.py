"""NoteForge 任务清理工具 — 删除超过保留期的 job 目录。
...
def cleanup_old_jobs(retention_days: int | None = None, dry_run: bool = False,
                     output_root: Path | str = Path("outputs")) -> tuple[int, int]:
    """
    删除超过保留期的 job 目录。
    ...
    jobs_dir = Path(output_root) / "jobs"
